...
std::string getCurrentDateTime()
{
    ...
    // Create a buffer to store formatted date and time
    char buffer[20];
...
std::string getValidVersionNumber()
{
    ...
    // Keep asking for input until valid 3-digit version number is provided
    while (!validInput)
    {
        std::cout << "Enter new version number (3 digits): ";
        std::cin >> version;

        // Check if input length is not 3 digits
        if (version.length() != 3)
        {
...
bool isFirstRun()
{
    ...
    // Return true if the log file doesn't exist (first run)
    return !logFile.good();
}
