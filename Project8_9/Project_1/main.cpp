...
void commentOutLogLine(const std::string &lineToComment)
{
    ...
    if (inputFile.is_open())
    {
        ...
        {
            // Check if the current line matches the line to comment out
            if (line.find(lineToComment) != std::string::npos)
            {
                commentOut = true;
            }

            // Comment out the current line if required
            if (commentOut)
            {
            ...
            // Check if the current line matches the _Prev line
            if (line.find(lineToComment + "_Prev") != std::string::npos)
            {
                commentOut = false; // Stop commenting out after _Prev line
            }
        }
        inputFile.close();
