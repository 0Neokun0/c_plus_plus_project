...
void updateMainVersion(std::vector<std::string> &fileLines, const std::string &filePath, const std::string &fileName)
{
    ...
    if (!currentMainVersionLine.empty() && currentMainVersionLine.find("_Prev") == std::string::npos)
    {
		...
		// Remove extra spaces from the component name
		std::string componentName = elements[0];
		componentName.erase(std::remove_if(componentName.begin(), componentName.end(), ::isspace), componentName.end());

		// Construct the new identifier with the new main version and existing revision number
		std::string newIdentifier = identifierPrefix + newMainVersion + "-" + newRevision;
		...
		std::string newMainVersionLine = "const Vi_Version " + componentName + "_Version( \"" + elements[0] + "\", \"" + elements[1] + "\", \"" + newMainVersion + "\", \"" + newRevision + "\", \"RELS_" + newIdentifier + "\", \"" + elements[5] + "\", \"" + elements[6] + "\");";
...
// Add "_Prev" and ".Prev" to the current main version line
if (!currentMainVersionLine.empty())
{
    size_t versionPos = currentMainVersionLine.find("_Version");
    if (versionPos != std::string::npos)
    {
        // Insert "_Prev" just before the opening parenthesis
        currentMainVersionLine.insert(versionPos, "_Prev");

        // Find the position of the opening quotation mark of the element "component"
        size_t componentStartPos = currentMainVersionLine.find("\"", versionPos);
        size_t componentEndPos = currentMainVersionLine.find("\"", componentStartPos + 1);
        if (componentStartPos != std::string::npos && componentEndPos != std::string::npos)
        {
            // Get the original component name
            std::string originalComponent = currentMainVersionLine.substr(componentStartPos + 1, componentEndPos - componentStartPos - 1);

            // Construct the modified component name with ".Prev"
            std::string modifiedComponent = originalComponent + ".Prev";

            // Replace the element "component" with the modified version containing ".Prev"
            currentMainVersionLine.replace(componentStartPos + 1, componentEndPos - componentStartPos - 1, modifiedComponent);
        }
    }
}

// Create a new version line with the updated main version and comments
std::string newMainVersionLine = currentMainVersionLine;
size_t versionPos = newMainVersionLine.find("_Version");
if (versionPos != std::string::npos)
{
    size_t revisionPos = newMainVersionLine.find("\"", versionPos);
    if (revisionPos != std::string::npos)
    {
        // Update the main version number and identifier
        newMainVersionLine.replace(versionPos + sizeof("_Prev") - 1, revisionPos - versionPos - sizeof("_Prev") + 2, "215");
        newMainVersionLine.replace(revisionPos + 1, sizeof("215-01") - 1, "215-01");
    }
}
...
for (std::string &line : fileLines)
{
    if (line.find("_Prev") != std::string::npos && line.find("//") != 0)
    {
		...
		updateLogFile(logFilePath, fileName, currentMainVersionLine, newVersionLine);

		// Display the updated main version line after logging
		std::cout << "Updated Main Version Line (after logging): " << newVersionLine << std::endl;
	}
}
